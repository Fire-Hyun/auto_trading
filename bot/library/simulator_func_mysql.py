...
class simulator_func_mysql():
    ...
    def variable_setting(self):
        ...
        self.use_min = False
        # 분별 시뮬레이션에서 일별 시뮬레이션과 동일하게 아침 9시에만 매수를 하고 싶은 경우 True, 9시가 아니어도 매수를 하고 싶은 경우 False
    ...
    def auto_trade_stock_realtime(self, min_date, date_rows_today, date_rows_yesterday):
        ...
        for j in range(self.len_df_realtime_daily_buy_list):
            if self.jango_check():
                ...
                # 아래는 영상 촬영 후 수정 하였습니다. open_price 를 가져오는 것을 분별/일별 시뮬레이션 구분하여 설정하였습니다.
    ...
    # 특정 종목의 시작가를 가져오는 함수(분별)
    def get_now_open_price_by_min(self, code_name, date):
        if self.is_min_craw_table_exist(code_name):
            sql = "select close from `" + code_name + "` where date like '%s' and open != 0 and volume !=0 limit 1"
            rows = self.engine_craw.execute(sql % (date + "%%")).fetchall()
            if len(rows) == 1:
                return rows[0][0]
            else:
                return False
    ...
    def get_now_close_price_by_min(self, code_name, min_date):
        sql = "select close from `" + code_name + "` where date = '%s' and open != 0 and volume !=0 order by sum_volume desc limit 1"
    ...
    def update_all_db_by_date(self, date):
        ...
        for j in range(len(possessed_code_name_list)):
            ...
            if open:
                self.db_to_all_item_present_price_update(possessed_code_name_list[j][0], open, clo5, clo10, clo20,
                                                         clo40,
    ...
    def trading_by_min(self, date_rows_today, date_rows_yesterday):
        ...
        if len(self.min_date_rows) != 0:
            ...
            for t in range(len(self.min_date_rows)):
                ...
                if self.is_simul_table_exist(self.db_name,"all_item_db") and len(self.get_data_from_possessed_item()) != 0:
                    ...
                    # 매도 함수
                    self.auto_trade_sell_stock(min)
    ...
    def simul_by_min(self, date_rows_today, date_rows_yesterday, i):
        ...
        if self.is_date_exist(date_rows_today) and self.is_date_exist(date_rows_yesterday):
            ...
            # 분별 시뮬레이팅 시작한다.
            self.trading_by_min(date_rows_today, date_rows_yesterday)
